...
public abstract class BaseTest {
    ...
    public void beforeSuite() throws Exception {
        ...
        switch(properties.getString("test.platform").toLowerCase()) {
            case "android":
                ...
                caps.setCapability(MobileCapabilityType.APP, properties.getString("app"));
        ...
        //navigateToAbsoluteUrl(properties.getString("environment.url"));
    }
    protected void navigateToAbsoluteUrl(String url) {
        driver.get(url);
        new WebDriverWait(driver, PAGE_LOAD_TIMEOUT).until(webDriver -> {
    ...
    protected void navigateToProductUrl(String relativePath) throws Exception {
        String currentUrl = driver.getCurrentUrl();
        navigateToAbsoluteUrl(currentUrl.substring(0, currentUrl.indexOf('/', 8)) + relativePath);
        String onloadFunction = properties.getString("product.onload.function");
        ((JavascriptExecutor) driver).executeScript("while(document.readyState != 'complete') {}; "+ onloadFunction);
    }
